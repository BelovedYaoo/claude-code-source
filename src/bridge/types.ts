...
/** Reusable API-only guidance appended to bridge auth errors. */
export const BRIDGE_LOGIN_INSTRUCTION =
  'Remote Control is unavailable in API-only mode. Use ANTHROPIC_API_KEY, configure apiKeyHelper, or use a supported third-party provider.'

/** Full error printed when `claude remote-control` is run without auth. */
export const BRIDGE_LOGIN_ERROR =
  'Error: Remote Control is unavailable in API-only mode.\n\n' +
